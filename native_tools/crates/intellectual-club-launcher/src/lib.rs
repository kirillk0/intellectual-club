...
pub fn run() -> Result<()> {
    run_with_entry(EntryPoint::Combined)
}

pub fn run_gui_entry() -> Result<()> {
    run_with_entry(EntryPoint::Gui)
}

pub fn run_cli_entry() -> Result<()> {
    run_with_entry(EntryPoint::Cli)
}
...
fn entry_action(entry: EntryPoint, command: Option<&CommandKind>) -> EntryAction {
    match (entry, command) {
        (EntryPoint::Combined, None) | (EntryPoint::Gui, None) => EntryAction::LaunchGui,
        (EntryPoint::Cli, None) => EntryAction::PrintHelp,
        (EntryPoint::Gui, Some(CommandKind::Daemon { .. })) => EntryAction::RunCommand,
        (EntryPoint::Gui, Some(_)) => EntryAction::Reject,
        (EntryPoint::Combined | EntryPoint::Cli, Some(_)) => EntryAction::RunCommand,
    }
}

fn run_with_entry(entry: EntryPoint) -> Result<()> {
    let args = Args::parse();

    match entry_action(entry, args.command.as_ref()) {
        EntryAction::PrintHelp => {
            Args::command().print_help()?;
            println!();
            return Ok(());
        }
        EntryAction::Reject => {
            bail!("CLI commands are available through intellectual-club-launcher-cli.exe")
        }
        EntryAction::LaunchGui | EntryAction::RunCommand => {}
    }
...
mod tests {
    ...
    #[test]
    fn windows_gui_entry_only_accepts_the_window_and_internal_daemon() {
        assert_eq!(entry_action(EntryPoint::Gui, None), EntryAction::LaunchGui);
        assert_eq!(
            entry_action(EntryPoint::Gui, Some(&CommandKind::Daemon { open: false })),
            EntryAction::RunCommand
        );
        assert_eq!(
            entry_action(EntryPoint::Gui, Some(&CommandKind::Status { json: false })),
            EntryAction::Reject
        );
    }

    #[test]
    fn windows_cli_entry_prints_help_without_a_command() {
        assert_eq!(entry_action(EntryPoint::Cli, None), EntryAction::PrintHelp);
        assert_eq!(
            entry_action(EntryPoint::Cli, Some(&CommandKind::Doctor)),
            EntryAction::RunCommand
        );
    }

    #[test]
    fn combined_entry_preserves_macos_launcher_behavior() {
        assert_eq!(
            entry_action(EntryPoint::Combined, None),
            EntryAction::LaunchGui
        );
        assert_eq!(
            entry_action(EntryPoint::Combined, Some(&CommandKind::Doctor)),
            EntryAction::RunCommand
        );
    }
