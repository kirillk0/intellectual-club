...
export function useChatMessageActions(params: Params) {
  ...
  const branchAssistantToNewChat = async (msg: ChatBranchMessage) => {
    ...
    try {
      ...
      publishChatChange({ operation: 'touch', id: nextChatId, meta: { reason: 'branch-new-chat' } });
  ...
  const moveBranchToNewChat = async (msg: ChatBranchMessage) => {
    ...
    try {
      ...
      const nextChatId = payload.chat?.id;
      if (!nextChatId) throw new Error('Missing chat id');
      publishChatChange({ operation: 'touch', id: params.chatId.value, meta: { reason: 'branch-moved' } });
      publishChatChange({ operation: 'touch', id: nextChatId, meta: { reason: 'branch-move-new-chat' } });
      await params.pushChatRoute(nextChatId);
    } catch (error) {
      console.error(error);
