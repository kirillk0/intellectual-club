defmodule IntellectualClub.Llm.Providers.Common.ChatCompletionsTrace do
  @moduledoc """
  Trace-oriented adapter for OpenAI-compatible Chat Completions streaming.

  It translates provider deltas into canonical runtime trace events.
  """
  ...
  def stream_generate(opts, emit) when is_map(opts) and is_function(emit, 1) do
    ...
    emit_old = fn
      ...
      other ->
        emit.({:response_error, %{provider: provider, error_text: inspect(other)}})
    end
