defmodule IntellectualClub.Llm.Providers.Common.ChatCompletions do
  @moduledoc """
  ...
  def stream_generate(opts, emit) when is_map(opts) and is_function(emit, 1) do
    ...
    provider = Map.fetch!(opts, :provider)
    logical_request = Map.get(opts, :request_payload, %{}) || %{}
    retryable_http_status_codes = retryable_http_status_codes(opts)
    ...
    case RequestHydration.hydrate(
      ...
      {:ok, wire_request} ->
        headers =
          ...
          ] ++ normalize_extra_headers(Map.get(opts, :extra_headers, []))
  ...
  defp retryable_http_status_codes(opts) when is_map(opts) do
    case Map.get(opts, :retryable_http_status_codes) do
      %MapSet{} = statuses ->
        statuses
  ...
  defp normalize_extra_headers(headers) when is_list(headers) do
  ...
  defp normalize_extra_headers(_headers), do: []

  defp stream_chat_completions(
         %Response{} = response,
         raw_request,
         url,
         provider,
    ...
    cond do
      stream_error_response?(raw_response, finish_reason) ->
        {status_code, error_text, retryable} =
          classify_stream_error(raw_response,
  ...
  defp classify_stream_error(raw_response, opts) when is_map(raw_response) and is_list(opts) do
    ...
    retryable_http_status_codes = Keyword.fetch!(opts, :retryable_http_status_codes)
