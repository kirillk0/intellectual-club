defmodule IntellectualClubWeb.Bff.ToolInstanceSharesController do
  @moduledoc """
  Owner-only sharing endpoints for direct tool instance group sharing.
  """
  ...
  def show(conn, %{"id" => id}) do
    with {:ok, actor} <- Helpers.require_actor(conn),
         {:ok, tool_instance_id} <- parse_resource_id(id),
         {:ok, state} <- Sharing.get_tool_instance_share_state(tool_instance_id, actor) do
      json(conn, state)
    else
  ...
  def update(conn, %{"id" => id} = params) do
    with {:ok, actor} <- Helpers.require_actor(conn),
         ...
         {:ok, state} <-
           Sharing.replace_tool_instance_share_state(tool_instance_id, group_ids, actor) do
      json(conn, state)
    else
      {:error, %Plug.Conn{} = conn} -> conn
      {:error, error} -> render_error(conn, error)
