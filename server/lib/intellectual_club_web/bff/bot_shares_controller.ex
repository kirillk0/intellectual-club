defmodule IntellectualClubWeb.Bff.BotSharesController do
  @moduledoc """
  Owner-only sharing endpoints for bot group sharing.
  """
  ...
  def show(conn, %{"id" => id}) do
    with {:ok, actor} <- Helpers.require_actor(conn),
         {:ok, bot_id} <- parse_resource_id(id),
         {:ok, state} <- Sharing.get_bot_share_state(bot_id, actor) do
      json(conn, state)
    else
  ...
  def update(conn, %{"id" => id} = params) do
    with {:ok, actor} <- Helpers.require_actor(conn),
         ...
         {:ok, tool_modes} <- parse_tool_modes(params),
         {:ok, state} <- Sharing.replace_bot_share_state(bot_id, group_ids, tool_modes, actor) do
      json(conn, state)
    else
      {:error, %Plug.Conn{} = conn} ->
        conn
  ...
  defp parse_tool_modes(params) do
    case Map.get(params, "tool_modes", %{}) do
      tool_modes when is_map(tool_modes) -> {:ok, tool_modes}
      _other -> {:error, {:validation, "tool_modes must be an object."}}
    end
  end
