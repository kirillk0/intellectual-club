defmodule IntellectualClubWeb.Bff.KnowledgeBlockSharesController do
  @moduledoc """
  Owner-only sharing endpoints for direct knowledge block group sharing.
  """
  ...
  def show(conn, %{"id" => id}) do
    with {:ok, actor} <- Helpers.require_actor(conn),
         {:ok, knowledge_block_id} <- parse_resource_id(id),
         {:ok, state} <-
           Sharing.get_knowledge_block_share_state(knowledge_block_id, actor) do
      json(conn, state)
    else
  ...
  def update(conn, %{"id" => id} = params) do
    with {:ok, actor} <- Helpers.require_actor(conn),
         ...
         {:ok, state} <-
           Sharing.replace_knowledge_block_share_state(
             knowledge_block_id,
             group_ids,
             actor
           ) do
