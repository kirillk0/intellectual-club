defmodule IntellectualClubWeb.CoreComponents do
  @moduledoc """
  ...
  def flash(assigns) do
    ...
    >
      <div class={[
        "relative flex gap-3 rounded-xl border p-4 shadow-lg",
        "bg-white/90 backdrop-blur supports-[backdrop-filter]:bg-white/70",
        @kind == :info && "border-blue-200/80 text-blue-950",
        @kind == :error && "border-red-200/80 text-red-950"
      ]}>
        <div class={[
          "mt-0.5 flex size-9 items-center justify-center rounded-lg",
          @kind == :info && "bg-blue-600/10",
          @kind == :error && "bg-red-600/10"
        ]}>
        ...
        <button
          type="button"
          class="group -mr-1 -mt-1 inline-flex size-8 items-center justify-center rounded-md hover:bg-zinc-900/5"
  ...
  attr :rest, :global,
    include:
      ~w(href navigate patch method download name value disabled form type phx-disable-with)
  ...
  attr :rest, :global,
    include: ~w(accept autocomplete capture cols disabled form list max maxlength min minlength
                ...
                phx-debounce phx-throttle phx-hook)
          ...
          class={
            @class ||
              [
                ...
                "shadow-sm focus:ring-2 focus:ring-zinc-900/20"
              ]
          }
          {@rest}
        />
        <span :if={@label} class="text-sm text-zinc-800">
          {@label}
          <span :if={@rest[:required]} class="align-super text-[10px] text-red-600" aria-hidden="true">
            *
          </span>
        </span>
      </label>
  ...
  def input(%{type: "select"} = assigns) do
    ...
    <div class="mb-3">
      <label class="block">
        <span :if={@label} class="block text-sm font-medium text-zinc-800">
          {@label}
          <span :if={@rest[:required]} class="align-super text-[10px] text-red-600" aria-hidden="true">
            *
          </span>
        ...
        <select
          ...
          class={[
            @class ||
              [
                "mt-1 block w-full rounded-md border bg-white px-3 py-2 text-sm shadow-sm",
                "border-zinc-300 text-zinc-900",
                "focus:border-zinc-900 focus:ring-2 focus:ring-zinc-900/20"
              ],
            @errors != [] &&
              (@error_class || "border-red-500 focus:border-red-500 focus:ring-red-500/20")
          ]}
          multiple={@multiple}
          {@rest}
        >
  ...
  def table(assigns) do
    ...
    <div class="overflow-hidden rounded-lg border border-zinc-200 bg-white shadow-sm">
      <table class="min-w-full divide-y divide-zinc-200 text-left text-sm">
        ...
        >
          <tr :for={row <- @rows} id={@row_id && @row_id.(row)} class="hover:bg-zinc-50/70">
            <td
              :for={col <- @col}
              phx-click={@row_click && @row_click.(row)}
              class={["px-4 py-3 text-zinc-800", @row_click && "cursor-pointer"]}
            >
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(IntellectualClubWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(IntellectualClubWeb.Gettext, "errors", msg, opts)
    end
  end
